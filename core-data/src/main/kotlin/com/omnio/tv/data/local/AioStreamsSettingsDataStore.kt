...
/**
 * Per-profile cache of the AIOStreams bridge-table row. We only persist the
 * UUID, manifest URL, and enable flag — provider creds live inside the
 * encrypted config payload upstream.
 */
@Singleton
class AioStreamsSettingsDataStore @Inject constructor(
    private val factory: ProfileDataStoreFactory,
    private val profileManager: ProfileManager,
) {
    companion object {
        private const val FEATURE = "aio_streams_settings"
    }
    ...
    suspend fun replaceAll(settings: AioStreamsSettings) {
        store().edit { prefs ->
    ...
    /**
     * Seeds an entire AIOStreams settings record for a specific profile (used
     * when provisioning a separate profile's AIOStreams config from a
     * different profile's context).
     */
