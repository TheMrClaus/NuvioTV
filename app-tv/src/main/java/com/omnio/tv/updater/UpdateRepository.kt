...
class UpdateRepository @Inject constructor(
    ...
    private suspend fun fetchReleaseUpdate(): AppUpdate {
        ...
        for (page in 1..MAX_RELEASE_PAGES) {
            ...
            val tvRelease = releases.firstOrNull(::isTvRelease) ?: continue
            return tvRelease.toAppUpdate()
        }

        error("No TV release found in the latest ${MAX_RELEASE_PAGES * 30} entries")
    }

    private fun isTvRelease(release: GitHubReleaseDto): Boolean {
    ...
    private fun UpdateManifestDto.toAppUpdate(): AppUpdate {
        ...
        val asset = AbiSelector.chooseBestApkAsset(manifestAssets)
            ?: error("No TV APK asset found in manifest $manifestTag")
